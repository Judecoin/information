...
`./judecoind --config-file=/etc/judecoin/judecoind.conf`
...
* `option-name=value`
* `valueless-option-name=1` for options that don't expect a value
* `# comment`
...
translates to:
    ...
    log-level=4
    stagenet=1     # use value "1" to enable valueless options like --stagenet
...
This config is tailored for production server usage.
    ...
    rpc-bind-port=16061            # Bind to the default port
    ...
    limit-rate-up=1048576     # 1048576 kB/s == 1GB/s; a raise from the default 2048 kB/s; contribute more to the P2P network
    limit-rate-down=1048576   # 1048576 kB/s == 1GB/s; a raise from the default 8192 kB/s; allows for faster initial sync
